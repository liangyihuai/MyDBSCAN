...
class DBSCAN private (
  ...
  private def train(rawVectors: RDD[RawVector]): DBSCAN = {
    ...
    val minimumRectanglesWithCount =
      rawVectors
        .map(rawVector => toMinimumBoundingRectangle(rawVector.vector))
        .map((_, 1))
    ...
    log.debug("Found partitions: ")
    localPartitions.foreach(p => log.debug(p.toString))
    ...
    val margins = rawVectors.context.broadcast(localMargins)

    // assign each point to its proper partition
    val duplicated = for {
      point <- rawVectors.map(rawVector => new DBSCANPoint(rawVector))
    ...
    log.debug("About to find adjacencies")
    ...
    log.debug("About to find all cluster ids")
      ...
      case ((id, map), clusterId) => {
        ...
        map.get(clusterId) match {
          case None => {
            ...
            log.debug(s"Connected clusters $connectedClusters")
    ...
    log.debug("Global Clusters")
    clusterIdToGlobalId.foreach(e => log.debug(e.toString))
    log.info(s"Total Clusters: ${localClusterIds.size}, Unique: $total")

    val clusterIds = rawVectors.context.broadcast(clusterIdToGlobalId)

    log.debug("About to relabel inner points")
    ...
    log.debug("About to relabel outer points")
